...
Bridges the React UI to the real Python model:
  ...
  POST /api/detect {scene,...} -> runs the trained model on a scene, returns
                                   detections + montage/preview image URLs
  GET  /outputs/<file>         -> serves the generated PNGs

The Keypoint R-CNN weights are loaded once at startup and reused per request.
Run:  python3 backend/server.py           (defaults to the jitter weights, port 8787)
...
MODEL = ds.load_model(DEFAULT_WEIGHTS)
print(f"model ready: {DEFAULT_WEIGHTS}")
