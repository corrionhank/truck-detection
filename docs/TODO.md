...
- [ ] **P0** **Signal-visibility check** — build the **Red−Blue band-difference** view on the **highest-traffic**
      scene (Seattle / a busy Stanwood date), *not* Bellingham. Does the moving echo actually appear at 3 m?
      **This gates everything below** ([APPROACHES.md](APPROACHES.md) §1).
- [ ] **P1** If the band-difference view is null on a high-traffic scene → queue a **basic-product order**
      when quota resets (ties to §2d) and re-test.
- [ ] **P2** Once echoes are confirmed: QGIS keypoint annotation layer (`vehicle_id` + B/R/G `sequence`,
      EPSG:32610) → COCO export; hand-label a handful of trucks to validate the format.
- [ ] **P2** Keypoint R-CNN replication ([APPROACHES.md](APPROACHES.md) §2.1) — start from COCO-pretrained,
      sweep small anchors (4/8/16/32/48 px), then train. Needs `torch` + `torchvision`.
...
## 3. Data & ops
- [x] Local-only project — **no git** (removed by request). `.gitignore` kept for reference / if you re-init later.
